...
public class GameObject
{
	...
	*/
	...
	/**
	 * A function to rotate an object's yaw.
	 * @param amt The angle in radians by which to rotate the object's yaw.
	 */
	public void yaw(float amt) {	// Rotate X
		Matrix4f rot = this.getWorldRotation();
		Vector3f up = this.getWorldUpVector();
		Matrix4f tempRot = (new Matrix4f()).rotation(amt, up);
		Matrix4f newRot = tempRot.mul(rot);
		this.setLocalRotation(newRot);
	}
	/**
	 * A function to rotate an object's yaw using global rotation.
	 * @param amt The angle in radians by which to rotate the object's yaw.
	 */
	public void globalYaw(float amt) {
		Vector3f worldUp = new Vector3f(0f, 1f, 0f);
		Matrix4f worldUpMat = new Matrix4f().rotation(amt, worldUp);
		Matrix4f currWorldRot = this.getWorldRotation();
		Matrix4f newWorldRot = worldUpMat.mul(currWorldRot, new Matrix4f());
		this.setLocalRotation(newWorldRot);
	}
	/**
	 * A function to rotate an object's pitch.
	 * @param amt The angle in radians by which to rotate the object's pitch.
	 */
	public void pitch(float amt) {	// rotate Y
		Matrix4f rot = this.getWorldRotation();
		Vector3f right = this.getWorldRightVector();
		Matrix4f tempRot = (new Matrix4f()).rotation(amt, right);
		Matrix4f newRot = tempRot.mul(rot);
		this.setLocalRotation(newRot);
	}
	/**
	 * A function to rotate an object's roll.
	 * @param amt The angle in radians by which to rotate the object's roll.
	 */
	public void roll(float amt) {	// Rotate Z
		Matrix4f rot = this.getWorldRotation();
		Vector3f fwd = this.getWorldForwardVector();
		Matrix4f tempRot = (new Matrix4f()).rotation(amt, fwd);
		Matrix4f newRot = tempRot.mul(rot);
		this.setLocalRotation(newRot);
	}
	/**
	 * A function to move an object along the Y axis.
	 * @param amt The amount by which to rotate the object along the Y axis.
	 */
	public void moveY(float amt) {	// Move Up/Down
		Vector3f up = this.getWorldUpVector();
		Vector3f loc = this.getWorldLocation();
		Vector3f newLoc = loc.add(up.mul(amt));
		this.setLocalLocation(newLoc);
	}
	/**
	 * A function to move an object along the X axis.
	 * @param amt The amount by which to rotate the object along the Y axis.
	 */
	public void moveX(float amt) { // Move Left/Right
		Vector3f right = this.getWorldRightVector();
		Vector3f loc = this.getWorldLocation();
		Vector3f newLoc = loc.add(right.mul(amt));
		this.setLocalLocation(newLoc);
	}
	/**
	 * A function to move an object along the Z axis.
	 * @param amt The amount by which to rotate the object along the Y axis.
	 */
	public void moveZ(float amt) {	// Move Forward/Backward
		Vector3f fwd = this.getWorldForwardVector();
		Vector3f loc = this.getWorldLocation();
		Vector3f newLoc = loc.add(fwd.mul(amt));
		this.setLocalLocation(newLoc);
	}
}